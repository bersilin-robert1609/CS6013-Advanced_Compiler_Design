...
if [[ $# == 1 ]] && [[ $1 == "-h" || $1 == "--help" ]]; then
    ...
    echo "custom            tests all files in test/input/custom_run and test/input/custom_norun folder" 
    ...
    then
        ...
        then
            ...
            then
                ...
                checkTacoJava=$(java --enable-preview -jar $dir/TacoJava2.jar < $MY_OUTPUT_DIR/$type/$file_name.java 2>/dev/null)
            else
                ...
                checkTacoJava=$(java --enable-preview -jar $dir/TacoJava2.jar < $MY_OUTPUT_DIR/$type/$file_name.java 2>$MY_OUTPUT_DIR/$type/$file_name.parseError)
            fi
        ...
        if [[ $notest == 1 ]]
        then
            if [[ $grammar_only == 0 && $type == "custom_run" ]]; then
                printProgress $cnt $total $progress_len
                java $MY_OUTPUT_DIR/$type/$file_name.java > $MY_OUTPUT_DIR/$type/$file_name.out 2>/dev/null
                clearAboveLine
            fi
        else
            if [[ $checkTacoJava == "Wrong TacoJava2" ]]
            then
                echo -e "----->" "${red}Wrong TacoJava2${clear}"
                if [[ $debug != 0 ]]; then
                    stop=1
                ...
                if [[ $grammar_only == 1 || $type == "custom_norun" ]]
                then
                    pass=$((pass+1))
                    echo -e "----->" "${green}Noice${clear} | Constant count = $checkTacoJava"
                else
                    printProgress $cnt $total $progress_len
